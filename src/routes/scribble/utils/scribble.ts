...
export function useUpdateScribbleMutation() {
  const targets = ["devto", "medium", "hashnode"] as const;
  const qc = useQueryClient();

  const publish_scribble_mutation = useSSM(async (ctx,vars: {
  ...
  const update_post_mutation = useMutation({
    mutationFn: (vars: { id: string; data: ScribblePostsUpdate }) => {
      return tryCatchWrapper(pb?.collection("scribble_posts").update(vars.id, vars.data));
    },
    onSuccess(data, variables, context) {
      if (data.error) {
